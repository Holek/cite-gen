<?php
/**
 * Polish {{Cytuj pismo}} template parser.
 *
...
class Journal_pl extends Template {
	...
	private function getAuthors($array)
	{
		...
		for($i = 1;$i<=count($array['last']) && $i < 6;$i++)
		{
			$this->appendParameter('nazwisko'.(($i==1)?'':$i),$array['last'][$i-1]);
			$this->appendParameter('imię'.(($i==1)?'':$i),$array['first'][$i-1]);
		...
		if (count($articles_array))
		{
			...
			{
				$this->appendParameter('autor link'.(($i==1)?'':$i),$author_link);
			}
		}
	}
